...
setwd('~/Desktop/Molecular Microbiome/Final Project/New_final_project/core-metrics-results/')
list.files()
...
meta<-read_q2metadata("finalmetadataEM2.tsv")
str(meta)
...
aov.evenness.sample_type = aov(pielou_evenness ~ sample_type, data=meta)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.sample_type)
...
TukeyHSD(aov.evenness.sample_type)
...
meta$sample_type.ord = factor(meta$sample_type, c("conten of caecum", "conten of jejunum"))
levels(meta$sample_type.ord)

#Plot
boxplot(pielou_evenness ~ sample_type.ord, data=meta, ylab="Peilou evenness")

evenness <- ggplot(meta, aes(sample_type.ord, pielou_evenness)) + 
...
shannon <- ggplot(meta, aes(sample_type.ord, )) + 
  geom_boxplot(aes(color = sample_type.ord)) + 
...
#Plot
boxplot(faith_pd ~ sample_type.ord, data=meta, ylab="Faith phylogenetic diversity")

boxplot(shannon_entropy ~ sample_type.ord, data=meta, ylab= "shannon_entropy")
ggsave("shannon.png", shannon_entropy, height = 3, width = 3)

boxplot(observed_features ~ sample_type.ord, data=meta, ylab="observed_features" )
ggsave("observedfeatures.png", observed_features, height = 3, width = 3)
# or with ggplot2

faith_pd <- ggplot(meta, aes(sample_type.ord, faith_pd)) + 
  geom_boxplot(aes(color = sample_type.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
